...
toolType * toolWallBasic::inputHandle(mapTool * mapTool)
{
		...
		PointMake(_ptMouse.x + mapTool->getCameraBufferX(), _ptMouse.y + mapTool->getCameraBufferY())))
	{
		mapTool->setIsToolWallBasic(false);
		SOUNDMANAGER->play("window_up", 0.4f);
		return new toolWallDungeon();
	}

	return nullptr;
}

void toolWallBasic::update(mapTool * mapTool)
{
...
void toolWallBasic::enter(mapTool * mapTool)
{
	mapTool->_popUp.img = IMAGEMANAGER->findImage("popUp");
	mapTool->_sampleTile.img = IMAGEMANAGER->findImage("wall_basic");
	mapTool->_currentTile.img = IMAGEMANAGER->findImage("wall_basic");
	...
	mapTool->setIsToolWallBasic(true);

	mapTool->_sampleTile.terrainFrameX = 0;
	mapTool->_sampleTile.terrainFrameY = 0;

	mapTool->_toolName.currentFrameX = 9;
	mapTool->_toolName.currentFrameY = 0;
}

void toolWallBasic::render(mapTool * mapTool)
{
...
void toolWallBasic::exit(mapTool * mapTool)
{
