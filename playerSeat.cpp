...
playerState * playerSeat::inputHandle(player * player)
{
...
void playerSeat::update(player * player)
{
	//플레이어 frame에 맞춘 방향 설정
	player->setPlayerCurrentFrameY((int)(player->_player.direction));
}

void playerSeat::enter(player * player)
{
	if (player->_player.playerCharacter == BLUE)
	{
		player->_player.img = IMAGEMANAGER->findImage("blue_seat_basic");
		player->_playerImageName = "blue_seat_basic";
		player->setPlayerImageMoveX(0);
		player->setPlayerImageMoveY(-11.75f);
	...
	{
		player->_player.img = IMAGEMANAGER->findImage("red_seat_basic");
		player->_playerImageName = "red_seat_basic";
		player->setPlayerImageMoveX(0);
...
void playerSeat::exit(player * player)
{
