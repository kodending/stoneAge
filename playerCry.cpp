...
playerState * playerCry::inputHandle(player * player)
{
...
void playerCry::update(player * player)
{
	//플레이어 frame에 맞춘 방향 설정
	player->setPlayerCurrentFrameY((int)(player->_player.direction));
}

void playerCry::enter(player * player)
{
	if (player->_player.playerCharacter == BLUE)
	{
		player->_player.img = IMAGEMANAGER->findImage("blue_cry_basic");
		player->_playerImageName = "blue_cry_basic";
		player->setPlayerCurrentFrameX(0);
		player->setPlayerCurrentFrameY((int)(player->_player.direction));

		if (player->getPlayerCurrentFrameY() == (int)DIR_SIX)
		{
			player->setPlayerImageMoveY(-36);
	...
	{
		player->_player.img = IMAGEMANAGER->findImage("red_cry_basic");
		player->_playerImageName = "red_cry_basic";
...
void playerCry::exit(player * player)
{
}
