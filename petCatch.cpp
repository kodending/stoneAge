...
petState * petCatch::inputHandle(pet * pet)
{
...
void petCatch::update(pet * pet)
{
	//플레이어 frame에 맞춘 방향 설정
	_elapsedSec += TIMEMANAGER->getElapsedTime();

	if (_elapsedSec >= 0.02f)
	{
		_elapsedSec -= 0.02f;

		pet->setCurrentFrameY(pet->getCurrentFrameY() + 1);

		if (pet->getCurrentFrameY() > pet->getImage()->getMaxFrameY())
		{
			pet->setCurrentFrameY(0);
		}
	}
...
void petCatch::enter(pet * pet)
{
	setPetImage(pet);
	pet->setCurrentFrameX(0);
}

void petCatch::exit(pet * pet)
{
}

void petCatch::setPetImage(pet * pet)
{
...
void petCatch::updatePetImage(pet * pet)
{
	if (pet->getPetName() == DURI)
	{
		if (pet->getPetDirection() == DIR_FIVE)
