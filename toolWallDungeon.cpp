...
void toolWallDungeon::update(mapTool * mapTool)
{
...
void toolWallDungeon::enter(mapTool * mapTool)
{
	mapTool->_popUp.img = IMAGEMANAGER->findImage("popUp");
	mapTool->_sampleTile.img = IMAGEMANAGER->findImage("wall_dungeon");
	mapTool->_currentTile.img = IMAGEMANAGER->findImage("wall_dungeon");
	...
	mapTool->setIsToolWallDungeon(true);

	mapTool->_sampleTile.terrainFrameX = 0;
	mapTool->_sampleTile.terrainFrameY = 0;

	mapTool->_toolName.currentFrameX = 8;
	mapTool->_toolName.currentFrameY = 0;
}

void toolWallDungeon::render(mapTool * mapTool)
{
	mapTool->_popUp.img->render(getMemDC(), mapTool->_popUp.PopUpRc.left, mapTool->_popUp.PopUpRc.top);

	mapTool->_sampleTile.img->frameRender(getMemDC(), mapTool->_sampleTile.rcTile.left, mapTool->_sampleTile.rcTile.top - 100,
...
void toolWallDungeon::exit(mapTool * mapTool)
{
