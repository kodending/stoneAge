...
toolType * toolObject::inputHandle(mapTool * mapTool)
{
		...
		PointMake(_ptMouse.x + mapTool->getCameraBufferX(), _ptMouse.y + mapTool->getCameraBufferY())))
	{
		mapTool->setIsToolObject(false);
		SOUNDMANAGER->play("window_up", 0.4f);
		return new toolFence();
	}

	return nullptr;
}

void toolObject::update(mapTool * mapTool)
{
...
void toolObject::enter(mapTool * mapTool)
{
	mapTool->_popUp.img = IMAGEMANAGER->findImage("popUp");
	mapTool->_sampleTile.img = IMAGEMANAGER->findImage("object");
	mapTool->_currentTile.img = IMAGEMANAGER->findImage("object");
...
void toolObject::exit(mapTool * mapTool)
{
