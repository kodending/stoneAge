...
playerState * playerAngry::inputHandle(player * player)
{
...
void playerAngry::update(player * player)
{
	//플레이어 frame에 맞춘 방향 설정
	player->setPlayerCurrentFrameY((int)(player->_player.direction));
}

void playerAngry::enter(player * player)
{
	if (player->_player.playerCharacter == BLUE)
	{
		player->_player.img = IMAGEMANAGER->findImage("blue_angry_basic");
		player->_playerImageName = "blue_angry_basic";
	...
	{
		player->_player.img = IMAGEMANAGER->findImage("red_angry_basic");
		player->_playerImageName = "red_angry_basic";
		player->setPlayerImageMoveX(0);
...
void playerAngry::exit(player * player)
{
}
